...
def search_query(query, index, positions, docID_url):
    """Main search function that returns ranked results and timing information"""
    ...
    ngram_postings = {}
    if os.path.exists(ngram_index_directory) and ngram_index:
        ngram_postings = get_ngram_postings(query_terms, ngram_index, ngram_positions)
    
    proximity_docs = set()
    if len(query_terms) > 1 and os.path.exists(position_index_directory):
        for i in range(len(query_terms) - 1):
